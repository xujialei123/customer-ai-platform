...
const fileGroups = [
  ...
  ['API 入口与基础设施', [
    ...
    ['apps/api/src/config/env.ts', 'UTF-8 读取根 .env；解析 LLM_PROVIDER（agnes/custom/openclaw）与可选 OpenClaw 路径。', '.env.example 和 model-config。'],
    ['apps/api/src/config/model-config.ts', '配置页 LLM/Embedding 热配置：JSON + .env + 内存；转发 RAG。', '/guide、openclaw.service、embedding。'],
  ...
  ['API Service 与 Worker', [
    ...
    ['apps/api/src/services/openclaw.service.ts', 'LLM 人设提示、多轮+摘要、本地兜底（读 model-config）。', 'ReplyWorker 与 /guide。'],
