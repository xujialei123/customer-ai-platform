...
const fileGroups = [
  ...
  ['API 入口与基础设施', [
    ...
    ['apps/api/src/config/env.ts', 'UTF-8 读取根 .env 并校验 API 环境变量。', '.env.example 和各 Service。'],
  ...
  ['API Service 与 Worker', [
    ...
    ['apps/api/src/services/message.service.ts', '消息去重、入库、摘要、outbound 确认和草稿关闭。', 'RPA outbound、ReplyDraft。'],
  ...
  ['RAG Service 兼容层', [
    ...
    ['services/rag-service/src/config/env.ts', 'Embedding、LLM、阈值和上传配置。', '.env.example。'],
    ...
    ['services/rag-service/src/providers/embedding.ts', 'Embedding Provider。', '向量维度。'],
  ...
  ['后台、文档、样例与生成物', [
    ...
    ['docs/project-walkthrough.md', '代码阅读顺序。', '目录与入口变化。'],
