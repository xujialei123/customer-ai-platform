...
该扩展运行在用户正常使用的 Chrome 中，不负责登录、不读取密码、不导出 Cookie。它只在已登录的美团经营宝或抖音来客页面中采集配置选择器命中的客户消息，并通过本机 WebSocket 发送到客服中台。同一扩展可同时打开两个平台标签页，配置按平台分别保存。
...
5. 正常打开并登录经营宝或抖音来客，然后点击扩展图标配置真实 DOM 选择器。弹窗会显示「当前页面：美团经营宝 / 抖音来客」。
...
2. 在 `chrome://extensions/` 重新加载本扩展，确保 `0.3.0` 的本地页面与抖音域名权限生效。
...
真实经营宝灰度测试时，如果配置了 `MEITUAN_RPA_ALLOWED_CUSTOMERS`，插件只会自动切换美团白名单客户的未读会话。抖音默认不做白名单过滤，所有客户均可监听与自动回复。
